            ...
            string path = null)
        {
            ...
            for (int i = 0; i < filenames.Length; ++i)
            {
                tasks[i] = Task.Factory.StartNew(index =>
                {
                    ...
                    var prediction = predictionEngine.Predict(new YoloV4BitmapData() { Image = bitmap });
                    var results = prediction.GetResults(classesNames, 0.2f);

                    foreach (var detectedObject in results)
                    {
                        if (cancToken.IsCancellationRequested)
                        {
                            return;
                        }
                        var tupleResult = new Tuple<string, YoloV4Result>(path, detectedObject);
                        resultsQueue.Enqueue(tupleResult);
                    }
                }, i);
            }
